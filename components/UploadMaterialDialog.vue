<template>
  <el-dialog title="上传资料(PDF)" :visible.sync="dialogVisible" width="600px" :before-close="handleClose"
    :show-close="false" :close-on-click-modal="false" class="upload-material-dialog" 
    custom-class="upload-material-dialog-custom">
...
export default {
  ...
  computed: {
    ...
    isUploadButtonDisabled() {
      return !this.agreementAccepted || this.fileList.length === 0 || this.isUploading || !this.fileType;
    },
    // 计算成功上传的文件数量
    successCount() {
  ...
  methods: {
    ...
    // 上传文件
    async handleUpload() {
      if (!this.agreementAccepted || this.fileList.length === 0 || this.isUploading || !this.fileType) return;
